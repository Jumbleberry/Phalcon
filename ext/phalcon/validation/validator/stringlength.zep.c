...
PHP_METHOD(Phalcon_Validation_Validator_StringLength, validate)
{
	...
	zval field, _2$$4, _3$$5;
	zval *validation, validation_sub, *field_param = NULL, isSetMin, isSetMax, value, length, message, minimum, maximum, label, replacePairs, code, _0, _4$$6, _5$$7, _6$$8, _7$$8, _8$$8, _9$$9, _10$$10, _11$$11, _12$$11, _13$$11, _14$$11;
	...
	if ((zephir_function_exists_ex(ZEND_STRL("mb_strlen")) == SUCCESS)) {
		...
		ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 210, &_2$$4);
		zephir_check_call_status();
	} else {
		zephir_cast_to_string(&_3$$5, &value);
		ZEPHIR_INIT_NVAR(&length);
		ZVAL_LONG(&length, zephir_fast_strlen_ev(&_3$$5));
	}
	if (zephir_is_true(&isSetMax)) {
		ZEPHIR_INIT_VAR(&_4$$6);
		ZVAL_STRING(&_4$$6, "max");
		ZEPHIR_CALL_METHOD(&maximum, this_ptr, "getoption", NULL, 0, &_4$$6);
		zephir_check_call_status();
		if (Z_TYPE_P(&maximum) == IS_ARRAY) {
			zephir_array_fetch(&_5$$7, &maximum, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/stringlength.zep", 116);
			ZEPHIR_CPY_WRT(&maximum, &_5$$7);
		}
		if (ZEPHIR_GT(&length, &maximum)) {
			ZEPHIR_INIT_VAR(&_6$$8);
			ZVAL_STRING(&_6$$8, "TooLong");
			ZEPHIR_INIT_VAR(&_7$$8);
			ZVAL_STRING(&_7$$8, "messageMaximum");
			ZEPHIR_CALL_METHOD(&message, this_ptr, "preparemessage", NULL, 0, validation, &field, &_6$$8, &_7$$8);
			zephir_check_call_status();
			...
			object_init_ex(&_6$$8, phalcon_validation_message_ce);
			ZEPHIR_CALL_FUNCTION(&_8$$8, "strtr", NULL, 26, &message, &replacePairs);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_7$$8);
			ZVAL_STRING(&_7$$8, "TooLong");
			ZEPHIR_CALL_METHOD(NULL, &_6$$8, "__construct", NULL, 443, &_8$$8, &field, &_7$$8, &code);
			zephir_check_call_status();
	...
	if (zephir_is_true(&isSetMin)) {
		...
		if (Z_TYPE_P(&minimum) == IS_ARRAY) {
			zephir_array_fetch(&_10$$10, &minimum, &field, PH_NOISY | PH_READONLY, "phalcon/validation/validator/stringlength.zep", 142);
		...
		if (ZEPHIR_LT(&length, &minimum)) {
			...
			zephir_array_update_string(&_13$$11, SL(":field"), &label, PH_COPY | PH_SEPARATE);
			zephir_array_update_string(&_13$$11, SL(":min"), &minimum, PH_COPY | PH_SEPARATE);
			ZEPHIR_CPY_WRT(&replacePairs, &_13$$11);
			ZEPHIR_INIT_NVAR(&_11$$11);
			object_init_ex(&_11$$11, phalcon_validation_message_ce);
			ZEPHIR_CALL_FUNCTION(&_14$$11, "strtr", NULL, 26, &message, &replacePairs);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_12$$11);
			ZVAL_STRING(&_12$$11, "TooShort");
			ZEPHIR_CALL_METHOD(NULL, &_11$$11, "__construct", NULL, 443, &_14$$11, &field, &_12$$11, &code);
			zephir_check_call_status();
