...
PHP_METHOD(Phalcon_Validation_Validator_Numericality, validate)
{
	...
	zval field, _1;
	zval *validation, validation_sub, *field_param = NULL, value, message, label, replacePairs, code, _0, _2, _3, _4, _6$$3, _7$$3, _8$$3;
	...
	ZEPHIR_INIT_VAR(&_4);
	ZVAL_STRING(&_4, "/^-?\\d+(?:[\\.,]\\d+)?$/");
	zephir_preg_match(&_3, &_4, &_1, &_0, 0, 0 , 0 );
	_5 = !zephir_is_true(&_3);
	if (!(_5)) {
		_5 = !(zephir_is_numeric(&value));
	}
	if (_5) {
		...
		ZEPHIR_CALL_METHOD(&message, this_ptr, "preparemessage", NULL, 0, validation, &field, &_6$$3);
		zephir_check_call_status();
		...
		object_init_ex(&_6$$3, phalcon_validation_message_ce);
		ZEPHIR_CALL_FUNCTION(&_7$$3, "strtr", NULL, 26, &message, &replacePairs);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_8$$3);
		ZVAL_STRING(&_8$$3, "Numericality");
		ZEPHIR_CALL_METHOD(NULL, &_6$$3, "__construct", NULL, 443, &_7$$3, &field, &_8$$3, &code);
		zephir_check_call_status();
