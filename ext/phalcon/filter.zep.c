...
{
	zval _3$$4, _14$$11;
	zend_bool _1$$3, _2$$3, _9$$9, _11$$9;
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval filter, _21$$22;
	zval *value, value_sub, *filter_param = NULL, filterObject, _0, _4$$5, _5$$5, _6$$6, _7$$8, _8$$9, _10$$9, _12$$9, _13$$10, _15$$11, _16$$13, _17$$13, _18$$20, _19$$21, _20$$22;
	zval *this_ptr = getThis();
	...
	if (zephir_array_isset_fetch(&filterObject, &_0, &filter, 0)) {
		...
		if (_1$$3) {
			_1$$3 = zephir_is_instance_of(&filterObject, SL("Closure"));
		}
		_2$$3 = _1$$3;
		if (!(_2$$3)) {
	...
	do {
		if (ZEPHIR_IS_STRING(&filter, "email")) {
			...
			zephir_check_call_status();
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", NULL, 200, value, &_5$$5);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "int")) {
			ZVAL_LONG(&_6$$6, 519);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", NULL, 200, value, &_6$$6);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "int!")) {
		...
		if (ZEPHIR_IS_STRING(&filter, "absint")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("abs", NULL, 201, &_7$$8);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "string")) {
			ZEPHIR_INIT_VAR(&_8$$9);
			ZEPHIR_GET_CONSTANT(&_8$$9, "PHP_MAJOR_VERSION");
			_9$$9 = ZEPHIR_LT_LONG(&_8$$9, 8);
			if (!(_9$$9)) {
				ZEPHIR_INIT_VAR(&_10$$9);
				ZEPHIR_GET_CONSTANT(&_10$$9, "PHP_MAJOR_VERSION");
				_11$$9 = ZEPHIR_IS_LONG_IDENTICAL(&_10$$9, 8);
				if (_11$$9) {
					ZEPHIR_INIT_VAR(&_12$$9);
					ZEPHIR_GET_CONSTANT(&_12$$9, "PHP_MINOR_VERSION");
					_11$$9 = ZEPHIR_IS_LONG_IDENTICAL(&_12$$9, 0);
				}
				_9$$9 = _11$$9;
			}
			if (_9$$9) {
				ZVAL_LONG(&_13$$10, 513);
				ZEPHIR_RETURN_CALL_FUNCTION("filter_var", NULL, 200, value, &_13$$10);
				zephir_check_call_status();
				RETURN_MM();
			}
			RETVAL_ZVAL(value, 1, 0);
		...
		if (ZEPHIR_IS_STRING(&filter, "float")) {
			...
			add_assoc_long_ex(&_14$$11, SL("flags"), 4096);
			ZVAL_LONG(&_15$$11, 520);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", NULL, 200, value, &_15$$11, &_14$$11);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "float!")) {
		...
		if (ZEPHIR_IS_STRING(&filter, "alphanum")) {
			...
			ZVAL_STRING(&_16$$13, "/[^A-Za-z0-9]/");
			ZEPHIR_INIT_VAR(&_17$$13);
			ZVAL_STRING(&_17$$13, "");
			ZEPHIR_RETURN_CALL_FUNCTION("preg_replace", NULL, 39, &_16$$13, &_17$$13, value);
			zephir_check_call_status();
			RETURN_MM();
		}
		...
		if (ZEPHIR_IS_STRING(&filter, "lower")) {
			if ((zephir_function_exists_ex(ZEND_STRL("mb_strtolower")) == SUCCESS)) {
				ZEPHIR_RETURN_CALL_FUNCTION("mb_strtolower", NULL, 16, value);
				zephir_check_call_status();
				RETURN_MM();
			}
		...
		if (ZEPHIR_IS_STRING(&filter, "url")) {
			ZVAL_LONG(&_18$$20, 518);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", NULL, 200, value, &_18$$20);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(&filter, "special_chars")) {
			ZVAL_LONG(&_19$$21, 515);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", NULL, 200, value, &_19$$21);
			zephir_check_call_status();
			RETURN_MM();
		}
		ZEPHIR_INIT_VAR(&_20$$22);
		object_init_ex(&_20$$22, phalcon_filter_exception_ce);
		ZEPHIR_INIT_VAR(&_21$$22);
		ZEPHIR_CONCAT_SVS(&_21$$22, "Sanitize filter '", &filter, "' is not supported");
		ZEPHIR_CALL_METHOD(NULL, &_20$$22, "__construct", NULL, 4, &_21$$22);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_20$$22, "phalcon/filter.zep", 230);
		ZEPHIR_MM_RESTORE();
		return;
	} while(0);
