...
PHP_METHOD(Phalcon_Dispatcher, dispatch)
{
	...
	while (1) {
		...
		if (hasEventsManager) {
			...
			if (EG(exception)) {
				...
				Z_ADDREF_P(&_56$$29);
				if (zephir_is_instance_of(&_56$$29, SL("Exception"))) {
					zend_clear_exception();
		...
		if ((zephir_method_exists_ex(&handler, ZEND_STRL("beforeexecuteroute")) == SUCCESS)) {
			...
			if (EG(exception)) {
				...
				Z_ADDREF_P(&_66$$34);
				if (zephir_is_instance_of(&_66$$34, SL("Exception"))) {
					zend_clear_exception();
		...
		if (ZEPHIR_IS_TRUE_IDENTICAL(&wasFresh)) {
			if ((zephir_method_exists_ex(&handler, ZEND_STRL("initialize")) == SUCCESS)) {
				...
				if (EG(exception)) {
					...
					Z_ADDREF_P(&_72$$40);
					if (zephir_is_instance_of(&_72$$40, SL("Exception"))) {
						zend_clear_exception();
			...
			if (zephir_is_true(&eventsManager)) {
				...
				if (EG(exception)) {
					...
					Z_ADDREF_P(&_82$$44);
					if (zephir_is_instance_of(&_82$$44, SL("Exception"))) {
						zend_clear_exception();
		...
		if (EG(exception)) {
			...
			Z_ADDREF_P(&_98$$10);
			if (zephir_is_instance_of(&_98$$10, SL("Exception"))) {
				zend_clear_exception();
		...
		if (hasEventsManager) {
			...
			if (EG(exception)) {
				...
				Z_ADDREF_P(&_108$$60);
				if (zephir_is_instance_of(&_108$$60, SL("Exception"))) {
					zend_clear_exception();
		...
		if ((zephir_method_exists_ex(&handler, ZEND_STRL("afterexecuteroute")) == SUCCESS)) {
			...
			if (EG(exception)) {
				...
				Z_ADDREF_P(&_116$$65);
				if (zephir_is_instance_of(&_116$$65, SL("Exception"))) {
					zend_clear_exception();
		...
		if (hasEventsManager) {
			...
			if (EG(exception)) {
				...
				Z_ADDREF_P(&_123$$70);
				if (zephir_is_instance_of(&_123$$70, SL("Exception"))) {
					zend_clear_exception();
	...
	if (hasEventsManager) {
		...
		if (EG(exception)) {
			...
			Z_ADDREF_P(&_129$$74);
			if (zephir_is_instance_of(&_129$$74, SL("Exception"))) {
				zend_clear_exception();
