...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct)
{
	...
	if (!ZEPHIR_IS_NULL(&callbacks)) {
		ZVAL_LONG(&_2$$4, 0);
		ZVAL_LONG(&_3$$4, ndocs);
		ZEPHIR_MAKE_REF(&_3$$4);
		ZEPHIR_CALL_FUNCTION(&yamlConfig, "yaml_parse_file", NULL, 140, &filePath, &_2$$4, &_3$$4, &callbacks);
		ZEPHIR_UNREF(&_3$$4);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&yamlConfig, "yaml_parse_file", NULL, 140, &filePath);
		zephir_check_call_status();
	}
