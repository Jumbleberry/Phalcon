...
PHP_METHOD(Phalcon_Logger_Adapter, setFormatter)
{
	...
	bool is_null_true = 1;
	ZEND_PARSE_PARAMETERS_START(1, 1)
		Z_PARAM_OBJECT_OF_CLASS(formatter, zephir_get_internal_ce(SL("phalcon\\logger\\formatterinterface")))
	ZEND_PARSE_PARAMETERS_END();
#endif
...
PHP_METHOD(Phalcon_Logger_Adapter, log)
{
	...
	if (ZEPHIR_GE(&_2, &toggledType)) {
		...
		if (zephir_is_true(&_3$$8)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_4$$9, "__construct", NULL, 22, &toggledMessage, &toggledType, &timestamp, &context);
			zephir_check_call_status();
