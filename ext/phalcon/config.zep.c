...
ZEPHIR_INIT_CLASS(Phalcon_Config)
{
	 ...
	 * @var string
	 */
	zend_declare_property_string(phalcon_config_ce, SL("pathDelimiter"), ".", ZEND_ACC_PROTECTED);
...
PHP_METHOD(Phalcon_Config, merge)
{
	...
	ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon_config_exception_ce, "Invalid data type for merge.", "phalcon/config.zep", 105);
	return;
}
...
PHP_METHOD(Phalcon_Config, path)
{
	...
	while (1) {
		...
		_5$$5 = 1 == ZEPHIR_IS_EMPTY(&config);
		if (!(_5$$5)) {
			_5$$5 = Z_TYPE_P(&config) != IS_OBJECT;
		}
		_6$$5 = _5$$5;
		if (!(_6$$5)) {
			_6$$5 = !((zephir_instance_of_ev(&config, phalcon_config_configinterface_ce)));
		}
		if (_6$$5) {
			break;
		}
...
PHP_METHOD(Phalcon_Config, toArray)
{
	...
	zephir_check_call_status();
	zephir_is_iterable(&data, 0, "phalcon/config.zep", 205);
...
PHP_METHOD(Phalcon_Config, internalMerge)
{
	...
	zephir_is_iterable(&target, 0, "phalcon/config.zep", 234);
	if (Z_TYPE_P(&target) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&target), _2, _3, _0)
		{
			...
			if (_5$$3) {
				...
				zephir_array_fetch(&_6$$3, &source, &key, PH_NOISY, "phalcon/config.zep", 224);
				_5$$3 = Z_TYPE_P(&_6$$3) == IS_ARRAY;
			}
			if (_5$$3) {
				zephir_array_fetch(&_8$$4, &source, &key, PH_NOISY | PH_READONLY, "phalcon/config.zep", 226);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$5) {
					...
					zephir_array_fetch(&_12$$5, &source, &key, PH_NOISY, "phalcon/config.zep", 224);
					_11$$5 = Z_TYPE_P(&_12$$5) == IS_ARRAY;
				}
				if (_11$$5) {
					zephir_array_fetch(&_14$$6, &source, &key, PH_NOISY | PH_READONLY, "phalcon/config.zep", 226);
...
PHP_METHOD(Phalcon_Config, setData)
{
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		object_init_ex(&_2$$3, phalcon_config_ce);
		zephir_read_property(&_3$$3, this_ptr, ZEND_STRL("insensitive"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_METHOD(NULL, &_2$$3, "__construct", NULL, 15, value, &_3$$3);
		zephir_check_call_status();
