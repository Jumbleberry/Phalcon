...
PHP_METHOD(Phalcon_Assets_Collection, add)
{
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "addresource", NULL, 97, resource);
...
PHP_METHOD(Phalcon_Assets_Collection, addInline)
{
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "addresource", NULL, 97, code);
	zephir_check_call_status();
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Assets_Collection, has)
{
	...
	bool is_null_true = 1;
	ZEND_PARSE_PARAMETERS_START(1, 1)
		Z_PARAM_OBJECT_OF_CLASS(resource, zephir_get_internal_ce(SL("phalcon\\assets\\resourceinterface")))
	ZEND_PARSE_PARAMETERS_END();
#endif
