...
PHP_METHOD(Phalcon_Assets_Manager, addResourceByType)
{
	...
	ZEND_PARSE_PARAMETERS_START(2, 2)
		Z_PARAM_STR(type)
		Z_PARAM_OBJECT_OF_CLASS(resource, zephir_get_internal_ce(SL("phalcon\\assets\\resource")))
	ZEND_PARSE_PARAMETERS_END();
#endif
	...
	if (!(zephir_array_isset_fetch(&collection, &_0, &type, 0))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &collection, "__construct", NULL, 102);
		zephir_check_call_status();
		zephir_update_property_array(this_ptr, SL("_collections"), &type, &collection);
	}
	ZEPHIR_CALL_METHOD(NULL, &collection, "add", NULL, 103, resource);
	zephir_check_call_status();
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Assets_Manager, addInlineCodeByType)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &collection, "addinline", NULL, 104, code);
	zephir_check_call_status();
	RETURN_THIS();
}
...
PHP_METHOD(Phalcon_Assets_Manager, output)
{
	...
	if (zephir_fast_count_int(&filters)) {
		...
		if (zephir_is_true(&join)) {
			...
			ZEPHIR_CALL_FUNCTION(&_3$$9, "is_dir", NULL, 105, &completeTargetPath);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Assets_Manager, outputCss)
{
	...
	ZEND_PARSE_PARAMETERS_START(0, 1)
		Z_PARAM_OPTIONAL
		Z_PARAM_ZVAL_OR_NULL(collectionName)
	ZEND_PARSE_PARAMETERS_END();
#endif
