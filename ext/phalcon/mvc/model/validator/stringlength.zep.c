...
PHP_METHOD(Phalcon_Mvc_Model_Validator_StringLength, validate)
{
	...
	zval *record, record_sub, field, isSetMin, isSetMax, value, length, maximum, minimum, message, _0, _2, _6$$8, _7$$9, _9$$9, _10$$11, _11$$12, _13$$12;
	...
	if ((zephir_function_exists_ex(ZEND_STRL("mb_strlen")) == SUCCESS)) {
		zephir_cast_to_string(&_4$$6, &value);
		ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 210, &_4$$6);
		zephir_check_call_status();
	} else {
		zephir_cast_to_string(&_5$$7, &value);
		ZEPHIR_INIT_NVAR(&length);
		ZVAL_LONG(&length, zephir_fast_strlen_ev(&_5$$7));
	}
	if (zephir_is_true(&isSetMax)) {
		ZEPHIR_INIT_VAR(&_6$$8);
		ZVAL_STRING(&_6$$8, "max");
		ZEPHIR_CALL_METHOD(&maximum, this_ptr, "getoption", NULL, 0, &_6$$8);
		zephir_check_call_status();
		if (ZEPHIR_GT(&length, &maximum)) {
			ZEPHIR_INIT_VAR(&_7$$9);
			ZVAL_STRING(&_7$$9, "messageMaximum");
			...
			zephir_array_update_string(&_8$$9, SL(":max"), &maximum, PH_COPY | PH_SEPARATE);
			ZEPHIR_CALL_FUNCTION(&_9$$9, "strtr", NULL, 26, &message, &_8$$9);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_7$$9);
			ZVAL_STRING(&_7$$9, "TooLong");
	...
	if (zephir_is_true(&isSetMin)) {
		...
		ZVAL_STRING(&_10$$11, "min");
		ZEPHIR_CALL_METHOD(&minimum, this_ptr, "getoption", NULL, 0, &_10$$11);
		zephir_check_call_status();
		if (ZEPHIR_LT(&length, &minimum)) {
			ZEPHIR_INIT_VAR(&_11$$12);
			ZVAL_STRING(&_11$$12, "messageMinimum");
			ZEPHIR_CALL_METHOD(&message, this_ptr, "getoption", NULL, 0, &_11$$12);
			zephir_check_call_status();
			if (ZEPHIR_IS_EMPTY(&message)) {
				ZEPHIR_INIT_NVAR(&message);
			...
			zephir_array_update_string(&_12$$12, SL(":field"), &field, PH_COPY | PH_SEPARATE);
			zephir_array_update_string(&_12$$12, SL(":min"), &minimum, PH_COPY | PH_SEPARATE);
			ZEPHIR_CALL_FUNCTION(&_13$$12, "strtr", NULL, 26, &message, &_12$$12);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(&_11$$12);
			ZVAL_STRING(&_11$$12, "TooShort");
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "appendmessage", NULL, 0, &_13$$12, &field, &_11$$12);
			zephir_check_call_status();
			RETURN_MM_BOOL(0);
		}
	}
