...
PHP_METHOD(Phalcon_Mvc_Micro, setService)
{
	...
	if (Z_TYPE_P(&dependencyInjector) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &dependencyInjector, "__construct", NULL, 299);
		zephir_check_call_status();
		zephir_update_property_zval(this_ptr, ZEND_STRL("_dependencyInjector"), &dependencyInjector);
	}
	if (shared) {
...
PHP_METHOD(Phalcon_Mvc_Micro, handle)
{
	...
	/* try_start_1: */
		...
		if (Z_TYPE_P(&matchedRoute) == IS_OBJECT) {
			...
			if (_21$$7) {
				_21$$7 = zephir_is_instance_of(&handler, SL("Closure"));
			}
			if (_21$$7) {
	...
	if (EG(exception)) {
		...
		Z_ADDREF_P(&_73);
		if (zephir_is_instance_of(&_73, SL("Exception"))) {
