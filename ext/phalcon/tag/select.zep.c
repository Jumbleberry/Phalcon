...
PHP_METHOD(Phalcon_Tag_Select, selectField)
{
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		ZEPHIR_CALL_SELF(&_8$$24, "_optionsfromresultset", &_9, 423, &options, &using, &value, &_11$$24);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_12$$26, "_optionsfromarray", &_13, 424, &options, &value, &_15$$26);
			zephir_check_call_status();
	...
	for (;_2->funcs->valid(_2) == SUCCESS && !EG(exception); _2->funcs->move_forward(_2)) {
		...
		if (Z_TYPE_P(using) == IS_ARRAY) {
			...
			if (Z_TYPE_P(value) == IS_ARRAY) {
				...
				zephir_cast_to_string(&_8$$16, &optionValue);
				ZEPHIR_CPY_WRT(&strOptionValue, &_8$$16);
				zephir_cast_to_string(&_9$$16, value);
...
PHP_METHOD(Phalcon_Tag_Select, _optionsFromArray)
{
	...
	if (Z_TYPE_P(data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(data), _2, _3, _0)
		{
			...
			ZEPHIR_CALL_FUNCTION(&escaped, "htmlspecialchars", &_4, 192, &optionValue);
			zephir_check_call_status();
			if (Z_TYPE_P(&optionText) == IS_ARRAY) {
				ZEPHIR_INIT_NVAR(&_5$$4);
				ZEPHIR_GET_CONSTANT(&_5$$4, "PHP_EOL");
				ZEPHIR_CALL_SELF(&_6$$4, "_optionsfromarray", &_7, 424, &optionText, value, closeOption);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(&optionText) == IS_ARRAY) {
					...
					ZEPHIR_CALL_SELF(&_17$$12, "_optionsfromarray", &_7, 424, &optionText, value, closeOption);
					zephir_check_call_status();
