...
PHP_METHOD(Phalcon_Validation, __construct)
{
	...
	if (zephir_fast_count_int(&validators)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "array_filter", NULL, 437, &validators, &_0$$3);
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "array_filter", NULL, 437, &validators, &_2$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, add)
{
	...
	ZEND_PARSE_PARAMETERS_START(2, 2)
		Z_PARAM_ZVAL(field)
		Z_PARAM_OBJECT_OF_CLASS(validator, zephir_get_internal_ce(SL("phalcon\\validation\\validatorinterface")))
	ZEND_PARSE_PARAMETERS_END();
#endif
...
PHP_METHOD(Phalcon_Validation, appendMessage)
{
	...
	bool is_null_true = 1;
	ZEND_PARSE_PARAMETERS_START(1, 1)
		Z_PARAM_OBJECT_OF_CLASS(message, zephir_get_internal_ce(SL("phalcon\\validation\\messageinterface")))
	ZEND_PARSE_PARAMETERS_END();
#endif
	...
	ZEPHIR_CALL_METHOD(NULL, &messages, "appendmessage", NULL, 438, message);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, ZEND_STRL("_messages"), &messages);
	RETURN_THIS();
}
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		...
		if (Z_TYPE_P(field) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(field), _0$$3)
			{
				ZEPHIR_INIT_NVAR(&singleField);
				ZVAL_COPY(&singleField, _0$$3);
				ZEPHIR_CALL_METHOD(&result, this_ptr, "prechecking", &_2, 439, &singleField, validator);
				zephir_check_call_status();
