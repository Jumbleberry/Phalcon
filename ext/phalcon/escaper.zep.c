...
PHP_METHOD(Phalcon_Escaper, detectEncoding)
{
	...
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _2)
		{
			...
			ZEPHIR_CALL_FUNCTION(&_4$$5, "mb_detect_encoding", &_5, 189, &str, &charset, &__$true);
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_6$$7, "mb_detect_encoding", &_5, 189, &str, &charset, &__$true);
				zephir_check_call_status();
	...
	ZEPHIR_INIT_NVAR(&charset);
	ZEPHIR_RETURN_CALL_FUNCTION("mb_detect_encoding", &_5, 189, &str);
	zephir_check_call_status();
	RETURN_MM();
}
